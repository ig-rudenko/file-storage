...
async def list_items(
    ...
    try:
        return await UserStorage(user.id).list_user_path(path or ".")
    except StorageException as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))


@router.post("/items/create-folder/{path:path}", status_code=status.HTTP_201_CREATED)
async def create_folder(path: str, user: User = Depends(get_current_user)):
    """Создает новую папку"""
    try:
        await UserStorage(user.id).create_folder(path)
...
async def rename_path(
    path: str, rename: RenameFile, user: User = Depends(get_current_user)
):
    """Переименовывает папку или файл"""
    try:
        await UserStorage(user.id).rename_file(path, rename.new_name)
...
async def delete_path(path: str, user: User = Depends(get_current_user)):
    """Удаляет папку или файл"""
    try:
        await UserStorage(user.id).delete_file(path)
...
async def upload_files(
    ...
    try:
        for file in files:
            await UserStorage(user.id).upload_file(path or ".", file)
        return Response(status_code=status.HTTP_200_OK)
    except StorageException as exc:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc))
